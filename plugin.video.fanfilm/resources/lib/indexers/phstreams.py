...
def localDirectory():
    getDirectory(
        "0", os.path.join(control.dataPath, phTest), "0", "0", "0", "0", "0", local=True
    )


def getDirectory(
    name, url, audio, image, fanart, playable, content, close=True, local=False
):
    if local == True:
    ...
    for popup in popups:
        try:
            ...
            addDirectoryItem(
                name, url, "openDialog", image, image, fanart, audio, "0", {}
            )
        except:
            pass
        ...
        try:
            ...
            if meta == True and content == "tvshows":
                try:
                    ...
                    data = cachemeta.get(
                        metaget.get_meta, 24, "tvshow", title, "", "", ""
                    )
        ...
        try:
            ...
            if meta == True and content == "movies":
                try:
                    ...
                    data = cachemeta.get(
                        metaget.get_meta, 24, "movie", title, "", "", year
                    )
...
def subDirectory(name, url, audio, image, fanart, playable, tvshow, content):
    ...
    try:
        if tvshow == "0" and not content == "episodes":
            raise Exception()
...
def getSearch():
    addDirectoryItem(
        "%s..." % control.lang(30702).encode("utf-8"),
        "0",
        "searchDirectory",
        "0",
        "0",
    ...
    addDirectoryItem(
        control.lang(30703).encode("utf-8"),
        "0",
        "clearSearch",
        "0",
        "0",
    ...
    try:
        ...
        for q in result:
            try:
                addDirectoryItem(
                    "%s..." % q, q, "searchDirectory2", "0", "0", "0", "0", "0", {}
                )
            except:
                pass
    except:
...
def searchDirectory(query=None):
    ...
    def search():
        return [
            x
            for y, x in enumerate((result + [query]))
        ...
        try:
            ...
            for dir in dirs:
                try:
                    ...
                    addDirectoryItem(
                        name, url, "ndmode", image, image, fanart2, "0", content, data
                    )
                except:
                    pass
    ...
    try:
        ...
        if "</source>" in url:
            ...
            for i in ["_mv", "_tv", "_mv_tv"]:
                try:
                    call = __import__(
                        "resources.lib.sources.%s%s" % (source, i),
                        globals(),
                        locals(),
                        ["object"],
                        -1,
            ...
            url = call.get_sources(
                url, d.hosthdfullDict, d.hostsdfullDict, d.hostlocDict
            )
            ...
            if type(url) == list and len(url) == 1:
                ...
                q = [
                    "[B]%s[/B] | %s" % (i["source"].upper(), i["quality"].upper())
                    for i in url
                ]
...
def addDirectoryItem(
    ...
    u = (
        "%s?name=%s&url=%s&audio=%s&image=%s&fanart=%s&playable=%s&tvshow=%s&content=%s&action=%s"
        % (
            sysaddon,
            urllib.quote_plus(name),
            urllib.quote_plus(url),
            urllib.quote_plus(audio),
            urllib.quote_plus(image),
            urllib.quote_plus(fanart),
            urllib.quote_plus(playable),
            str(tvshow),
            str(content),
            str(action),
        )
    ...
    if content == "movies" and not isFolder == True:
        ...
        cm.append(
            (
                ...
                % (
                    sysaddon,
                    urllib.quote_plus(downloadFile),
                    urllib.quote_plus(url),
                    urllib.quote_plus(image),
                ),
            )
...
def cleantitle(name):
    ...
    name = re.sub(
        "\s\[COLOR.+?\].+?\[/COLOR\]|\[/COLOR\]\[COLOR.+?\]\s.+?\[/COLOR\]|\[COLOR.+?\]|\[/COLOR\]",
        "",
        name,
    )
