...
def nhlDirectory():
    ...
    addDirectoryItem(
        control.lang(30751).encode("utf-8"), "Scoreboard", "nhlScoreboard", "0", "0"
    )
    addDirectoryItem(
        control.lang(30752).encode("utf-8"), "Archived", "nhlArchives", "0", "0"
    )
        ...
        try:
            ...
            try:
                name = "%s at %s  [COLOR gold](%s)[/COLOR]  [COLOR red](%s)[/COLOR]" % (
                    item["awayTeamName"],
                    item["homeTeamName"],
                    (procTimezone(5, est)).strftime("%H:%M"),
                    (procTimezone(5, est)).strftime("%Y-%m-%d"),
                )
            except:
                name = (
                    "%s at %s  [COLOR gold](%s)[/COLOR]  [COLOR red](%s-%s-%s)[/COLOR]"
                    % (
                        item["awayTeamName"],
                        item["homeTeamName"],
                        est[3],
...
def nhlStreams(name, url):
    try:
        ...
        url = (
            "http://smb.cdnak.neulion.com/fs/nhl/mobile/feed_new/data/streams/%s/ipad/%s_%s.json"
            % (url[0], url[1], url[2])
        )
    ...
    if finish == "false":
        for i in l1:
            addDirectoryItem(
                i["name"], i["url"], "nhlResolve", i["image"], "0", isFolder=False
            )
    else:
        for i in l2:
            addDirectoryItem(
...
def nhlResolve(url):
    try:
        try:
            url, selectGame, side = re.compile("(.+?)x0xe(.+?)x0xe(.+?)$").findall(url)[
                0
            ]
        except:
