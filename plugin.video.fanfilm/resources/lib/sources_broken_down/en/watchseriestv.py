...
class source:
    ...
    def tvshow(self, imdb, tvdb, tvshowtitle, localtvshowtitle, aliases, year):
        try:
            ...
            url = self.base_link + self.search_link % (
                tvshowtitle.replace(" ", "+").replace("-", "+").replace("++", "+")
            )
        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    pass
                sea = client.parseDOM(
                    item, "div", attrs={"class": "season-table-season"}
                )[0]
                epi = client.parseDOM(item, "div", attrs={"class": "season-table-ep"})[
                    0
                ]
                if cleantitle.get(season) in cleantitle.get(sea) and cleantitle.get(
                    episode
                ) in cleantitle.get(epi):
                    url = self.base_link + url
                    return url
            return
        except:
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            page = client.request(url)
            links = re.compile(
                '<a rel="nofollow" target="blank" href="(.+?)"', re.DOTALL
            ).findall(page)
            for link in links:
