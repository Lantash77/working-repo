...
class source:
    ...
    def resolve(self, url):
        try:
            ...
            p1 = cfScraper.post(
                "http://123123movies.net/ip.file/swf/plugins/ipplugins.php", data=post
            ).content
            p1 = json.loads(p1)
            p2 = cfScraper.get(
                "http://123123movies.net/ip.file/swf/ipplayer/ipplayer.php?u=%s&s=%s&n=0"
                % (p1["s"], urldata["data-server"])
            ).content
            p2 = json.loads(p2)
            p3 = cfScraper.get(
                "http://123123movies.net/ip.file/swf/ipplayer/api.php?hash=%s"
                % (p2["hash"])
            ).content
            p3 = json.loads(p3)
            n = p3["status"]
            if n == False:
                p2 = cfScraper.get(
                    "http://123123movies.net/ip.file/swf/ipplayer/ipplayer.php?u=%s&s=%s&n=1"
                    % (p1["s"], urldata["data-server"])
                ).content
                p2 = json.loads(p2)
