...
class source:
    ...
    def movie(self, imdb, title, localtitle, aliases, year):
        try:
            ...
            possibleTitles = set(
                ...
                + tuple(
                    (alias["title"].lower() for alias in aliases) if aliases else ()
                )
            )
            return self._getSearchData(
                lowerTitle, possibleTitles, year, self._createSession(), isMovie=True
            )
        except:
    ...
    def tvshow(self, imdb, tvdb, tvshowtitle, localtvshowtitle, aliases, year):
        try:
            ...
            return self._getSearchData(
                lowerTitle, possibleTitles, year, self._createSession(), isMovie=False
            )
        except:
            log_utils.log("PrimewireGR - Exception", 1)
            return
    ...
    def episode(self, data, imdb, tvdb, title, premiered, season, episode):
        try:
            ...
            if r.ok:
                ...
                firstEpisodeDIV = mainDIV.find(
                    "div", {"class": "show_season", "data-id": season}
                )
                # Filter the episode HTML entries to find the one that represents the episode we're after.
                episodeDIV = next(
                    (
                        element
                        for element in firstEpisodeDIV.next_siblings
                        if not isinstance(element, NavigableString)
                        and next(element.a.strings, "").strip("E ") == episode
                    ),
                    None,
                )
                if episodeDIV:
        ...
        try:
            ...
            for hostBlock in mainDIV.findAll("tbody"):
                ...
                if "onclick" in hostBlock.a.attrs:
                    ...
                    quality = (
                        "SD"
                        if ("cam" not in qualityClass and "ts" not in qualityClass)
                        else "CAM"
                    )
                    ...
                    unresolvedData = {
                        ...
                        + hostBlock.a["href"],  # Not yet usable, see resolve().
                        "UA": data["UA"],
                        "cookies": session.cookies.get_dict(),
                        "referer": pageURL,
                    }
    ...
    def _getSearchData(self, query, possibleTitles, year, session, isMovie):
        try:
            searchURL = (
                self.BASE_URL
                + ("/?" if isMovie else "/?tv=&")
                + urlencode({"search_keywords": query})
            )
            ...
            for resultDIV in mainDIV.findAll(
                "div", {"class": "index_item"}, recursive=False
            ):
