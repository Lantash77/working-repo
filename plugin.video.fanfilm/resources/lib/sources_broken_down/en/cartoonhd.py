...
from oathscrapers import parse_qs, urljoin, urlparse, urlencode, quote, unquote_plus
    ...
    def searchShow(self, title, season, episode, aliases, headers):
        try:
            for alias in aliases:
                ...
                url = client.request(
                    url, headers=headers, output="geturl", timeout="10"
                )
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            if "tvshowtitle" in data:
                url = self.searchShow(
                    title, int(data["season"]), int(data["episode"]), aliases, headers
                )
            else:
            ...
            self.base_link = client.request(
                self.base_link, headers={"User-Agent": client.agent()}, output="geturl"
            )
            ...
            tim = (
                str(int(time.time()))
                if six.PY2
                else six.ensure_binary(str(int(time.time())))
            )
