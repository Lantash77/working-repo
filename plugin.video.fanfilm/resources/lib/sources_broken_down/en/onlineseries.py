...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            posts = [
                dom_parser.parse_dom(i.content, "a", req="href") for i in posts if i
            ]
            posts = [
                (i[0].attrs["href"], re.sub("<.+?>", "", i[0].content))
                for i in posts
                if i
            ]
            posts = [
                ...
                for i in posts
                if (
                    cleantitle.get_simple(i[1].split(hdlr)[0]) == cleantitle.get(title)
                    and hdlr.lower() in i[1].lower()
                )
            ]
    ...
    def _get_sources(self, url):
        try:
            ...
            try:
                size = re.findall(
                    "((?:\d+\.\d+|\d+\,\d+|\d+)\s*(?:GiB|MiB|GB|MB))", item
                )[0]
