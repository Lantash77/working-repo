...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            url = urljoin(
                self.base_link, self.search_link % quote_plus(cleantitle.query(title))
            )
            ...
            if "tvshowtitle" in data:
                ...
                match = re.compile(
                    'class="post-item.+?href="(.+?)" title="(.+?)"', re.DOTALL
                ).findall(html)
                for url, item_name in match:
                    if (
                        cleantitle.getsearch(title).lower()
                ...
                match = re.compile(
                    '<div class="thumbnail".+?href="(.+?)" title="(.+?)"', re.DOTALL
                ).findall(html)
                ...
                for url, item_name in match:
                    if (
                        ...
                        Regex = re.compile(
                            'href="/download.php.+?link=(.+?)"', re.DOTALL
                        ).findall(result)
