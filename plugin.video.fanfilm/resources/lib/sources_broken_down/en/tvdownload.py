...
class source:
    ...
    def episode(self, url, imdb, tvdb, title, premiered, season, episode):
        try:
            ...
            query = "%s+s%02de%02d" % (
                cleantitle.geturl(tvshowtitle).replace("-", "+"),
                int(season),
                int(episode),
            )
            url2 = urlparse.urljoin(self.base_link, self.search_link % (query))
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            _headers = {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:61.0) Gecko/20100101 Firefox/72.0"
            }
            r = client.request(url, headers=_headers)
            posts = client.parseDOM(r, "h2", attrs={"class": "title"})
            posts = zip(
                client.parseDOM(posts, "a", ret="title"),
