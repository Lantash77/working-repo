...
class source:
    ...
    def episode(self, url, imdb, tvdb, title, premiered, season, episode):
        try:
            ...
            url = urlparse.urljoin(
                self.base_link, (self.search_link % (clean_title, url["year"]))
            )
    ...
    def resolve(self, url):
        try:
            ...
            cfScraper.headers.update(
                {"Referer": urldata["url"], "X-Requested-With": "XMLHttpRequest"}
            )
            p1 = cfScraper.post(
                "http://123movieshubz.com/ip.file/swf/plugins/ipplugins.php", data=post
            ).content
            p1 = json.loads(p1)
            p2 = cfScraper.get(
                "http://123movieshubz.com/ip.file/swf/ipplayer/ipplayer.php?u=%s&s=%s&n=0"
                % (p1["s"], urldata["data-server"])
            ).content
            p2 = json.loads(p2)
            p3 = cfScraper.get(
                "http://123movieshubz.com/ip.file/swf/ipplayer/api.php?hash=%s"
                % (p2["hash"])
            ).content
            p3 = json.loads(p3)
            n = p3["status"]
            if n == False:
                p2 = cfScraper.get(
                    "http://123movieshubz.com/ip.file/swf/ipplayer/ipplayer.php?u=%s&s=%s&n=1"
                    % (p1["s"], urldata["data-server"])
                ).content
                p2 = json.loads(p2)
