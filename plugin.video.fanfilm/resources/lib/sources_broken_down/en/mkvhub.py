...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            for i in items:
                threads.append(
                    workers.Thread(self._get_sources, i[0], i[1], hostDict, hostprDict)
                )
    ...
    def _get_sources(self, url, name, hostDict, hostprDict):
        try:
            ...
            urls = [
                (
                    client.parseDOM(
                        result, "a", ret="href", attrs={"class": "dbuttn watch"}
                    )[0],
                    client.parseDOM(
                        result, "a", ret="href", attrs={"class": "dbuttn blue"}
                    )[0],
                    client.parseDOM(
                        result, "a", ret="href", attrs={"class": "dbuttn magnet"}
                    )[0],
                )
            ]
        ...
        for url in urls[0]:
            try:
                ...
                if "linkomark" in url:
                    p_link = client.parseDOM(
                        r, "link", attrs={"rel": "canonical"}, ret="href"
                    )[0]
                    ...
                    links = client.parseDOM(
                        p_data, "a", ret="href", attrs={"target": "_blank"}
                    )
