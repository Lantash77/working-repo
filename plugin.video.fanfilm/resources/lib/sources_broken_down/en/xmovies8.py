...
class source:
    ...
    def movie(self, imdb, title, localtitle, aliases, year):
        try:
            ...
            r = [
                (i[0].attrs["href"], re.search("Release:\s*(\d+)", i[0].content))
                for i in r
                if i
            ]
    ...
    def episode(self, url, imdb, tvdb, title, premiered, season, episode):
        try:
            ...
            season_url = [
                u for u in seasons_urls if u.endswith("-season-%d.html" % int(season))
            ][0]

            r2 = cfScraper.get(season_url).content
            r2 = ensure_text(r2, errors="ignore")
            episodes = client.parseDOM(
                r2, "div", attrs={"id": "details", "class": "section-box"}
            )[0]
            episodes = client.parseDOM(episodes, "a", ret="href")
            url = [e for e in episodes if e.endswith("episode-%d.html" % int(episode))][
                0
            ]
