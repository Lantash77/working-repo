...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            r = client.parseDOM(
                r, "div", attrs={"style": "display:table;width:100%;text-align:left"}
            )
            ...
            for post in posts:
                ...
                name = url.split("&dn=")[1]
                if not query in cleantitle.get_title(name):
                    continue

                quality, info = source_utils.get_release_quality(name, url)
                try:
                    size = client.parseDOM(
                        post, "span", attrs={"class": "torrent_size"}
                    )[0]
