...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            query = (
                "%s S%02dE%02d" % (title, int(data["season"]), int(data["episode"]))
                if "tvshowtitle" in data
                else "%s %s" % (title, data["year"])
            )
            query = re.sub("(\\\|/| -|:|;|\*|\?|\"|'|<|>|\|)", " ", query)
            ...
            for post in posts:
                try:
                    ...
                    r = client.parseDOM(r, "div", attrs={"class": "entry-content cf"})[
                        0
                    ]

                    if "tvshowtitle" in data:
                        z = zip(
                            re.findall(r"<p><b>(%s.+?)</b>" % title, r, re.I | re.S),
                            re.findall(r"<ul>(.+?)</ul>", r, re.S),
                        )
                        for f in z:
                            u = re.findall(r"\'(http.+?)\'", f[1]) + re.findall(
                                r"\"(http.+?)\"", f[1]
                            )
