...
class source:
    def __init__(self):
        ...
        self.headers = {
            ...
            % ensure_text(
                base64.b64encode(ensure_binary("%s:%s" % (self.user, self.password)))
            ),
    ...
    def episode(self, url, imdb, tvdb, title, premiered, season, episode):
        try:
            ...
            r = [
                (str(i["id"]), str(i["season"]), str(i["number"]), str(i["airdate"]))
                for i in r
            ]
            ...
            url = [
                i
                for i in r
                if season == "%01d" % int(i[1]) and episode == "%01d" % int(i[2])
            ]
