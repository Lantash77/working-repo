...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            year = (
                re.findall("(\d{4})", data["premiered"])[0]
            ...
            if not r and "tvshowtitle" in data:
                ...
                query = query.replace("  ", " ")
                query = query.replace(" ", "-")
                r, _base_link = client.list_request(
            ...
            for loopCount in range(0, 2):
                if loopCount == 1 or (r is None and "tvshowtitle" in data):
                    ...
                    query = (
                        query.replace("&", " and ").replace("  ", " ").replace(" ", "-")
            ...
            for item in items:
                try:
                    ...
                    host2 = host.strip('"')
                    host = re.findall(
                        "([\w]+[.][\w]+)$", urlparse(host2.strip().lower()).netloc
                    )[0]
