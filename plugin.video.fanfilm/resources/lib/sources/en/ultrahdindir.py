...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            r = [
                (dom_parser.parse_dom(i, "div", attrs={"class": "news-title"}))
                for i in r
                if data["imdb"] in i
            ]
            ...
            for item in r:
                try:
                    ...
                    try:
                        details = client.parseDOM(
                            data, "div", attrs={"class": "text_spoiler"}
                        )[0]
                    except:
                        details = None
                    if details:
                        _zip = zip(
                            [u for u in urls if u.startswith("https://turbobit")],
                            re.findall(r"General : (.+?)<br", details),
                            re.findall(r"Length : (.+?) for", details),
                        )
                    else:
                        _zip = zip(
                            [u for u in urls if u.startswith("https://turbobit")],
                            re.findall(
                                r"/uploads/0-0-vip-(.+?).jpg", data, re.I | re.S
                            ),
                        )
                    ...
                    for z in _zip:
                        try:
                            ...
                            name = ensure_str(client.replaceHTMLCodes(z[1])).replace(
                                "dual", " dual "
                            )
