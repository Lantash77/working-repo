...
class source:
    ...
    def search(self, title, year):
        try:
            ...
            r = [
                (urljoin(self.base_link, i[0]))
                for i in r
                if cleantitle.get(title) in cleantitle.get(i[1]) and year in i[1]
            ]
            if r:
                return r[0]
            else:
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            r = [
                (i.content, urljoin(self.base_link, i.attrs["href"]))
                for i in r
                if i and i.content != "Watch"
            ]
            if hdlr2 != "":
    ...
    def _get_sources(self, name, url):
        try:
            ...
            urls = [
                i
                for i in urls
                if not i.endswith((".rar", ".zip", ".iso", ".idx", ".sub", ".srt"))
            ]
            for url in urls:
                ...
                try:
                    size = re.findall(
                        "((?:\d+\.\d+|\d+\,\d+|\d+)\s*(?:GiB|MiB|GB|MB))", name
                    )[0]
