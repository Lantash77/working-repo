...
class source:
    def __init__(self):
        ...
        self.meta_search_link = (
            "/api/plugins/metasearch?api_key=%s&q=%s&cached=yes"
        ...
        self.user_name = control.addon("plugin.video.fanfilm").getSetting(
            "furk.user_name"
        )
        self.user_pass = control.addon("plugin.video.fanfilm").getSetting(
            "furk.user_pass"
        )
        self.api_key = control.addon("plugin.video.fanfilm").getSetting("furk.api")
        self.search_limit = control.addon("plugin.video.fanfilm").getSetting(
            "furk.limit"
        )
        self.files = []
        ...
        try:
            ...
            if api_key == "":
                if self.user_name == "" or self.user_pass == "":
                    ...
                    if p["status"] == "ok":
                        api_key = p["api_key"]
                        control.addon("plugin.video.fanfilm").setSetting(
                            "furk.api", api_key
                        )
                    else:
                        pass
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            if content_type == "movie":
                title = (
                    url["title"].replace(":", " ").replace(" ", "+").replace("&", "and")
                )
                title = title.replace("'", "")
                year = url["year"]
