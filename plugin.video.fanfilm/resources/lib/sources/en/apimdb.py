...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            urls = client.parseDOM(
                posts, "div", attrs={"class": "server"}, ret="data-src"
            )
            urls = [
                urljoin(self.base_link, url) if url.startswith("/") else url
                for url in urls
            ]
            ...
            for url in urls:
                try:
                    ...
                    if valid:
                        ...
                        h in host for h in ['googledrive2', 'googledrive9', 'vip-', 'hls-']
                    ):
                        ...
                        for url in links:
                            if url.startswith("http"):
                                ...
                                if valid:
                                    direct = (
                                        True if url.endswith(direct_stream) else False
                                    )
                                    sources.append(
                                        {
                                            "source": host,
