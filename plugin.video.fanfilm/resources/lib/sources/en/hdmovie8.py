...
class source:
    ...
    def episode(self, url, imdb, tvdb, title, premiered, season, episode):
        try:
            ...
            tvshowtitle = url
            url = urljoin(
                self.base_link, "/episodes/%s-%sx%s/" % (tvshowtitle, season, episode)
            )
            return url
        except:
            return
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            thesources = re.compile("<tbody>(.+?)</tbody>", re.DOTALL).findall(
                sourcePage
            )[0]
            links = re.compile(
                "<a href='(.+?)' target='_blank'>Download</a>", re.DOTALL
            ).findall(thesources)
            for link in links:
                linkPage = ensure_text(cfScraper.get(link).content, errors="replace")
                vlink = re.compile(
                    '<a id="link" rel="nofollow" href="(.+?)" class="btn"', re.DOTALL
                ).findall(linkPage)
                for zlink in vlink:
