        ...
        try:
            ...
            for searchtitle in searchtitles:
                ...
                result = client.parseDOM(
                    result, "ul", attrs={"class": "resultsList hits"}
                )
                ...
                for content in items:
                    try:
                        orgtitle = client.parseDOM(
                            content,
                            "div",
                            attrs={"class": "filmPreview__originalTitle"},
                        )[0]
                    except:
                        orgtitle = "0"
                        pass
                ...
                years = client.parseDOM(
                    result, "span", attrs={"class": "filmPreview__year"}
                )
        ...
        try:
            ...
            for searchtitle in searchtitles:
                ...
                response = requests.get(
                    self.base_link + self.search_serial % searchtitle
                )
                result = response.text
                result = client.parseDOM(
                    result, "ul", attrs={"class": "resultsList hits"}
                ...
                for item in zip(titles, ids, years, orgtitles):
                    ...
                    if self.contains_all_wors(teststring, words) and str(year) in [
                        str(x) for x in range(int(year) - 1, int(year) + 1)
                    ]:
                        return (
                            f_title,
                            f_id,
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            response = requests.post(
                "http://fboost.pl/api/api.php", data=data, headers=headers
            )
            ...
            for code in zip(content[u"link"], content[u"wersja"]):
                ...
                test = requests.post(
                    "http://fboost.pl/api/player.php?src=%s" % code[0]
                ).text
