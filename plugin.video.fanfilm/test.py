...
def worker_thread(provider_name, provider_source):
    ...
    try:
        ...
        if not getattr(provider_source, "unit_test", False):
            ...
            # Run movie Call
            url = provider_source.movie(
                movie_info["imdb"],
                movie_info["title"],
                movie_info["localtitle"],
                movie_info["aliases"],
                movie_info["year"],
            )
            if url is None:
...
if __name__ == "__main__":
    ...
    if test_mode == 1:
        for provider in provider_list:
            workers.append(
                threading.Thread(target=worker_thread, args=(provider[0], provider[1]))
            )
    ...
    if test_mode == 1:
        all_sources = [
            source
            for sources in passed_providers
    ...
    if test_mode == 1:
        ...
        print(
            "Skipped Providers: %s"
            % (len(provider_list) - (len(passed_providers) + len(failed_providers)))
        )
    else:
