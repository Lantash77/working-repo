...
class ResolveUrlChecker:
    ...
    def getOfficialResolveUrlVersion(self):
        try:
            ...
            resolveUrlVersion = re.findall(r"ResolveURL\" version=\"(.*?)\"", content)[
                0
            ]
            return resolveUrlVersion
        except Exception as e:
            xbmc.log(
                "[SCRIPT.MODULE.PTW] Can't get resolveUrl version from official repo",
                xbmc.LOGERROR,
            )
    ...
    def getInstalledResolveUrlVersion(self):
        try:
            ...
            xbmc.log(
                "[SCRIPT.MODULE.PTW] Can't get resolveUrl version from kodi installation",
                xbmc.LOGERROR,
            )
    ...
    def checkVersion(self):
        if self.LocalVersion != self.OfficialVersion:
            ...
            ret = dialog.yesno(
                "Niekompatybilna wersja zależności",
    ...
    def removeResolveUrl(self):
        addonPath = xbmcvfs.translatePath(
            "special://home/addons/script.module.resolveurl"
        )
    ...
    def closeKodi(self):
        choice = xbmcgui.Dialog().ok(
            "Zamknięcie", "Teraz Kodi zostanie zamknięte aby zatwierdzić zmiany"
        )
        if choice == 1:
            os._exit(1)
        else:
