...
class CloudScraper(Session):
    ...
    def is_Challenge_Request(self, resp):
        ...
        if self.is_New_IUAM_Challenge(resp):
            self.simpleException(
                CloudflareChallengeError, "Detected the new Cloudflare challenge."
            )
    ...
    def reCaptcha_Challenge_Response(self, provider, provider_params, body, url):
        ...
        captchaResponse = reCaptcha.dynamicImport(provider.lower()).solveCaptcha(
    ...
    def Challenge_Response(self, resp, **kwargs):
        if self.is_reCaptcha_Challenge(resp):
            ...
            if (
                not self.recaptcha
                or not isinstance(self.recaptcha, dict)
                or not self.recaptcha.get("provider")
            ):
                self.simpleException(
                    CloudflareReCaptchaProvider,
            ...
            submit_url = self.reCaptcha_Challenge_Response(
                self.recaptcha.get("provider"), self.recaptcha, resp.text, resp.url
            )
        else:
            # ------------------------------------------------------------------------------- #
