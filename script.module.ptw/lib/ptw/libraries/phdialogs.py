...
def openDialog(image, audio):
    ...
    path = xbmcvfs.translatePath(
        os.path.join(
            "special://home/addons/plugin.video.phstreams/resources/skins/DefaultSkin",
            "media",
        )
    )
    ...
    if xbmc.getCondVisibility("system.platform.ios"):
        if not xbmc.getCondVisibility("system.platform.atv"):
            popup = dialog(
                ...
                "DefaultSkin",
                close_time=20,
                logo_path="%s/resources/skins/DefaultSkin/media/Logo/"
                % xbmcaddon.Addon().getAddonInfo("path"),
            )
...
def downloadFile(url, dest, silent=False, cookie=None):
    try:
        ...
        if not silent:
            ...
            dialog.ok(
                "Phoenix Streams",
                "Report any errors  at " + supportsite,
...
class dialog(xbmcgui.WindowXMLDialog):
    ...
    def onInit(self):
        xbmc.Player().play(
            "%s/resources/skins/DefaultSkin/media/tempsound.mp3"
            % xbmcaddon.Addon().getAddonInfo("path")
        )  # Music
    ...
    def onAction(self, action):
        if action in [5, 6, 7, 9, 10, 92, 117] or action.getButtonCode() in [
            275,
            257,
            261,
        ]:
