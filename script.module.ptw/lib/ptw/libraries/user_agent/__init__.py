...
class User_Agent:
    ...
    def loadHeaders(self, user_agents, user_agent_version):
        if (
            user_agents.get(self.browser)
            .get("releases")
            .get(user_agent_version)
            .get("headers")
        ):
            self.headers = (
                user_agents.get(self.browser)
    ...
    def filterAgents(self, releases):
        ...
        for release in releases:
            if self.mobile and releases[release]["User-Agent"]["mobile"]:
                filtered[release] = (
                    filtered.get(release, [])
                    + releases[release]["User-Agent"]["mobile"]
                )

            if self.desktop and releases[release]["User-Agent"]["desktop"]:
                filtered[release] = (
                    filtered.get(release, [])
                    + releases[release]["User-Agent"]["desktop"]
                )
            ...
            for release in user_agents[browser]["releases"]:
                for platform in ["mobile", "desktop"]:
                    if re.search(
                        ...
                        " ".join(
                            user_agents[browser]["releases"][release]["User-Agent"][
                                platform
                            ]
                        ),
                    ):
