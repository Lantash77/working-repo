...
class captchaSolver(reCaptcha):
    ...
    def checkErrorStatus(response, request_type):
        ...
        if response.json().get("status") == 0 and response.json().get(
            "request"
        ) in errors.get(request_type):
    ...
    def requestJob(self, jobID):
        ...
        if response:
            ...
            raise reCaptchaTimeout("2Captcha: Error failed to solve reCaptcha.")
    ...
    def requestSolve(self, captchaType, url, siteKey):
        ...
        data = {"key": self.api_key, "pageurl": url, "json": 1, "soft_id": 5507698}
        ...
        if response:
            ...
            raise reCaptchaBadJobID("2Captcha: Error no job id was returned.")
    ...
    def getCaptchaAnswer(self, captchaType, url, siteKey, reCaptchaParams):
        ...
        if not reCaptchaParams.get("api_key"):
            raise reCaptchaParameter("2Captcha: Missing api_key parameter.")
        ...
        try:
            ...
            raise reCaptchaTimeout(
                "2Captcha: reCaptcha solve took to long to execute job id {}, aborting.".format(
                    jobID
                )
            )
